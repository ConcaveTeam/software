...
TrackMono::TrackMono()
  : it(nh)
  , sub_left(it.subscribe("/stereo/left/image_rect", 1, &TrackMono::callback, this))
  , pub_img(it.advertise("monotrack", 1))
  , pub(nh.advertise<geometry_msgs::PointStamped>("target", 1))
{
  left_saved.header.frame_id = "left_cam";
}
...
TrackMono::TrackMono()
        : it(nh)
        , sub_right(it.subscribe("/stereo/right/image_rect", 1, &TrackMono::callback, this))
        , pub_img(it.advertise("monotrack", 1))
        , pub(nh.advertise<geometry_msgs::PointStamped>("target", 1))
{
